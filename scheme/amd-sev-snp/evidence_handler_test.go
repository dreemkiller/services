...
func Test_GetTrustAnchorID_ok(t *testing.T) {
	reportBytes, err := os.ReadFile("test/attestation_report.dat")
	...
	expectedTaID := "AMD-SEV-SNP://Milan/"
...
func Test_SynthKeysFromTrustAnchor_ok(t *testing.T) {
	...
	require.NoError(t, err)
	expectedKey := "AMD-SEV-SNP://Milan/"

	scheme := &EvidenceHandler{}
	key_list, err := scheme.SynthKeysFromTrustAnchor("Milan", &endors)
	require.NoError(t, err)
...
func Test_SynthKeysFromRefValue_ok(t *testing.T) {
	...
	scheme := &EvidenceHandler{}
	key_list, err := scheme.SynthKeysFromRefValue("Milan", &endors)
	require.NoError(t, err)
	assert.Equal(t, expectedKey, key_list[0])
}
